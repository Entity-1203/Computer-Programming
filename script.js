...
// VALIDATION
function validateInputs(id, name, category, quantity, price) {
...
productForm.addEventListener("submit", function (e) {
  ...
  try {
    ...
    // DUPLICATE CHECK — only checks against OTHER entries, not the one being edited
    ...
    const productData = { id, name, category, quantity, price };
    ...
    if (editIndex !== -1) {
      ...
      errorMessage.textContent = "Product updated successfully.";
...
// DELETE PRODUCT — opens the modal
...
// WAIT FOR DOM TO BE READY before attaching modal button listeners
...
searchInput.addEventListener("input", function () {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const productId = products[i].id.toLowerCase();
    ...
    for (let j = 0; j < sortedProducts.length - i - 1; j++) {
      ...
      if (sortValue === "name") {
        ...
        shouldSwap =
          ...
          sortedProducts[j + 1].name.toLowerCase();
